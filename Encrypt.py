...
import os
def AES(key,iv):
    f=open(os.path.join(os.getcwd()+"/Segments","0.txt"),"r", encoding="utf-8")
    content=f.read()
    f.close()
...
def BlowFish(key,iv):
    f=open(os.path.join(os.getcwd()+"/Segments","1.txt"),"r", encoding="utf-8")
    content=f.read()
    f.close()
    content=content.encode()
    b=len(content)
...
def TrippleDES(key,iv):
    f=open(os.path.join(os.getcwd()+"/Segments","2.txt"),"r", encoding="utf-8");
    content=f.read();
    f.close();
    content=content.encode()
    b=len(content);
...
def IDEA(key,iv):
	f=open(os.path.join(os.getcwd()+"/Segments","3.txt"),"r", encoding="utf-8")
	content=f.read()
	f.close()
	content=content.encode()
	b=len(content)
...
def EFernet(key):
    f=open(os.path.join(os.getcwd()+"/Segments","4.txt"),"r", encoding="utf-8")
    content=f.read()
    f.close()
    content=content.encode()
    fer = Fernet(key)
